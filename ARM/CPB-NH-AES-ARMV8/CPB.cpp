...
char infoString[] = "NH-AES ARM CORTEX A76 RaspberryPi5";  /* Each AE implementation must have a global one */
...
void update_funtion(uint32x4_t X,uint32x4_t Y, uint8x16_t *roundKeys_zero,uint8x16_t *roundKeys_ones, uint64x2_t * output){
    
    // cifrado de dos rondas de aes con llaves de 0 y 1
    uint32x4_t U = vreinterpretq_u32_u8 (AES_Encrypt_rounds( vreinterpretq_u8_u32(X), roundKeys_zero, 2)); 
    uint32x4_t V = vreinterpretq_u32_u8 (AES_Encrypt_rounds( vreinterpretq_u8_u32(Y), roundKeys_ones, 2)); 
    ...
    result[3] = vmull_u32(high_U, high_V);
...
void NH_AES(const uint8_t* input, uint8_t* tag, const uint8_t* key_1,const uint8_t* key_2, const uint64_t lenght){
    ...
    uint8x16_t roundKeys_ones[Nr + 1];
    ...
    for (i = 0; i < size-1; i=i+2){
        //Funcion H
        uint8x16_t generate_key_x = AES_Encrypt_rounds( vreinterpretq_u8_u32(index), roundKeys_1, 8); 
        index=vaddq_u32(index, const_vec);
        
        uint8x16_t generate_key_y = AES_Encrypt_rounds( vreinterpretq_u8_u32(index), roundKeys_1, 8); 
        ...
        update_funtion(X, Y, roundKeys_zero,roundKeys_ones, output);
...
int main(int argc, char **argv)
{
    ...
    uint8x16_t roundKeys_1[Nr + 1];
	char outbuf[(MAX_ITER*20)+1024];
	uint64_t iter_list[2048]; /* Populate w/ test lengths, -1 terminated */
	char *outp = outbuf;
	int iters, i, j;
    int64_t len;
	double Hz,sec;
	...
	i=0;
	len = iter_list[0];
    for (size_t j = 0; j< size_message; j++){
        plaintext[j]=j;                
    }
	...
	while (len >= 0) {
		...
		do {
            ...
            NH_AES(plaintext, tag, key, key, len);
