...
#define size_message 32768   // Tamaño del mensaje a procesar
...
//     EliMAC(plaintext, tag, key_1, key_2, size_message);
...
//     // // print_array_16bits((uint8_t*)&roundKeys[10], 16);

//     // // Realizar el cifrado
//     // // AES_Encrypt(ciphertext, plaintext, roundKeys);
//     // AES_Encrypt_rounds(ciphertext, plaintext, roundKeys,2);
...
int main(int argc, char **argv)
{
	...
	while (len >= 0) {
		...
		do {
            EliMAC(plaintext, tag, key, key, len);
			c = clock();
			for (j = 0; j < iters; j++) {
                EliMAC(plaintext, tag, key, key, len);
