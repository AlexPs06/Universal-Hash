...
char infoString[] = "Mulimixer ARM CORTEX A76 RaspberryPi5";  /* Each AE implementation must have a global one */
...
void update_funtion(uint32x4_t X,uint32x4_t Y, uint8x16_t *roundKeys_zero, uint64x2_t * output, int suma_vectorizada){
    
    
    uint32x4_t U=vdupq_n_u32(0);
    uint32x4_t V=vdupq_n_u32(0);

    if (suma_vectorizada)
    {
        for (int i = 0; i < 4; i++){
            U[i] = vaddvq_u32(X)-X[(i+3)%4];
            V[i] = vaddvq_u32(Y)-Y[(i+3)%4];
        }
    }else{
        for (int i = 0; i < 4; i++){
            U[i] = X[i]+X[(i+1)%4]+X[(i+2)%4];
            V[i] = Y[i]+Y[(i+1)%4]+Y[(i+2)%4];
        }
...
void multimixer128(const uint8_t* input, uint8_t* tag, const uint8_t* key_1,const uint8_t* key_2, const uint64_t lenght, int condicion){
    ...
    for (i = 0; i < size-1; i=i+2){
        ...
        update_funtion(X, Y, roundKeys_zero, output,condicion);
...
int main(int argc, char **argv)
{
	...
	while (len >= 0) {
		...
		do {
            multimixer128(plaintext, tag, key, key, len,suma_vectorizada);
